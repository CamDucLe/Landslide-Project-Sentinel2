...
def main():
    ...
    generator = DataGenerator(data_dir='../dataset/train', batch_size=BATCH_SIZE, train_ratio=0.8, band_opt=1) # band_opt: 0->14, 1->27, 2->9

    if is_train == 1:
        model = createModel()
        print('### START TRAINING PROCESS ###')
        trainModel(model=model, stored_dir=stored_dir, generator=generator)
    else:

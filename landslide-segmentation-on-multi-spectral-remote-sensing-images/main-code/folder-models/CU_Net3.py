...
def AttentionBlock(x):
    ...
    p1  = GlobalAveragePooling2D(keepdims=False)(x) # C
    p2  = GlobalMaxPooling2D(keepdims=False)(x)     # C
    n_neuron = p1.shape[-1] # C
    dense1  = Dense(int(n_neuron/2), activation='selu') # C/2
    p1      = dense1(p1)
    p2      = dense1(p2)
    dense2  = Dense(n_neuron, activation='relu')    # C 
    p1      = dense2(p1)
    p2      = dense2(p2)
    cam     = sigmoid(p1+p2)
    
    cam     = Reshape((1,1,cam.shape[-1]))(cam)     # 1x1xC
    cbam    = x * cam                               # WxHxC * 1x1xC -> WxHxC

    # SAM - Spatial Attention module
    m1  = tf.math.reduce_mean(cbam, axis=-1)                    # WxH
    m1  = Reshape((cbam.shape[-3],cbam.shape[-2],1))(m1)        # WxHx1
    m2  = tf.math.reduce_max(cbam, axis=-1)                     # WxH
    m2  = Reshape((cbam.shape[-3],cbam.shape[-2],1))(m2)        # WxHx1
    sam = Concatenate(axis=-1)([m1, m2])                        # WxHx2
    sam = Conv2D(filters=1, kernel_size=4, strides=(1, 1), padding='same')(sam) # WxHx1
    sam = sigmoid(sam)                                          # WxHx1
...
def getModel(pre_trained_model=None):
    network = getCUNet(input_shape=(128,128,21))

...
def getRNet(input_shape=(128,128,23), num_classes=2, dropout_ratio=0.2):
    ...
    c0 = Conv2D(128, kernel_size=3, strides=1, padding='same',activation='selu')(inputs)

    # Contracting path
    c1 = ResidualBlock(c0, 128) # 128
    p1 = Conv2D(128, strides=2, kernel_size=2, padding='same', activation='selu')(c1)
    
    c2 = ResidualBlock(p1, 128)     # 128
    p2 = Conv2D(128, strides=2, kernel_size=2, padding='same', activation='selu')(c2)
    
    c3 = ResidualBlock(p2, 128)     # 128
    p3 = Conv2D(128, strides=2, kernel_size=2, padding='same', activation='selu')(c3)
    
    c4 = ResidualBlock(p3, 128)     # 128
    p4 = Conv2D(128, strides=2, kernel_size=2, padding='same', activation='selu')(c4)
    
    # Bottom
    b = ResidualBlock(p4, 128)      # 128

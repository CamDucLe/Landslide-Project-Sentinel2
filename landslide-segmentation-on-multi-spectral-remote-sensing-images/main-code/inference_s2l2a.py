...
def readTifFile(tif_file_head):
    ...
    ## read with rasterio
    #tif_file_head = '/var/data/storage/datasets/image/01_AI_for_Earth/02_slandslide/04_data_gaia_S2/2018/33TUM/tmp/33_T_UM_2018_9_S2A_33TUM_20180926_0_L2A_B'
    tif_file_tail = '.tif'
...
def predictLandslide(model, x_seq, img_index, img_per_rowcol=86):
    ## make prediction
    ...
    y_pred = tf.math.reduce_sum(mask, axis=[1,2])
    
    ## image has less than 10 predicted landslide pixel will be mark as 0 else 1
    tmp    = tf.where(y_pred < 5, 0, 1 ).numpy()  # y_pred < 10
...
def predictOneImage(model, l_img):
    ...
    for h in range(batch_h[0]):
        ...
        for w in range(batch_w[0]):
            ...
            if (count == 50) or ((h == batch_h[0]-1) and (w == batch_w[0]-1)): # count == 30 for 2080 and count == 50 for TITAN
                re_list.extend(predictLandslide(model, x_seq, img_index))
                count = 0
...
def savePrediction(file_name, pred_list, save_opt='pkl'):
    ...
    file_name = '../predictions_folder/' +  file_name + file_tail
...
def doInference():
    ...
    """
    ## load model
    model_path = '../results/39-mono-10-RAUNet-multi/model.h5'
    model = tf.keras.models.load_model( filepath=model_path,
                                        custom_objects={ 'FocalLoss': FocalLoss, 'IOULoss': IOULoss})
    
    ## load large image
    data_dir = '/var/data/storage/datasets/image/01_AI_for_Earth/02_slandslide/04_data_gaia_S2/'
    ## loop thourgh each year
    for i in ['2018','2019','2020','2021','2022']:
        img_dir = data_dir + i + '/33TUM/tmp/' # add year to path
        ...
        for file_name in file_names:
            ...
            ## read large image with rasterio
            l_img = readTifFile(tif_file_head)
            #print('large image shape: ',l_img.shape)    
            
            ## cut, predict a large image and return a list of positions and landslide masks
            pred_list = predictOneImage(model, l_img)

            ## save prediction
            savePrediction(file_name, pred_list)
            
            ## below code for testing only
            #with open('../predictions_folder/' + file_name + '.pkl', 'rb') as f:
            #    lst = pickle.load(f)
            #    print(file_name, '  len : ', len(lst))
            #exit()

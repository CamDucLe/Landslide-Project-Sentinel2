...
def doSegmentation2(batch_num):
    ## initialize data generator
    generator = DataGenerator(data_dir='data', batch_size=1, train_ratio=1., band_opt=1, is_multi_resolution=1) # band_opt: 0->14, 1->23, 2->9
    
    ## load model
    model = loadModel("k2.h5")
...
with gr.Blocks(title="Landslide-Segmentation-Demo") as intf:
    with gr.Column(variant="panel"):
        with gr.Row():
            slider = gr.Slider(0, 20, step=1, label="Change slider to see different landslide images, masks and predictions of models trained from fold 1 and 2 (only 21 examples ^^)")
        ...
        with gr.Row():    
            gallery_1 = gr.Gallery( columns=4 )
        with gr.Row():    
            gallery_2 = gr.Gallery( columns=4 )            

    btn.click(doSegmentation, slider, gallery_1)
    btn.click(doSegmentation2, slider, gallery_2)

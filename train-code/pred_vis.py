...
def pred_vis():
    ...
    generator = DataGenerator(data_dir='../dataset/train', batch_size=BATCH_SIZE, train_ratio=1, band_opt=1, is_multi_resolution=is_multi_resolution) # band_opt: 0->14, 1->23, 2->9

    ## load model
    model = loadModel(best_model_file)  

    ## run Data_Generator
    x_test, y_true_test_256, y_true_test_128, y_true_test_64, n_imgs = generator.getBatch(batch_num=901, is_aug=False, is_train=True, is_cutmix=False)
    ...
    visualizeOneMask(tf.math.argmax(y_pred_test[1][0],axis=-1)) 

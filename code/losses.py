...
class EntropyLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        # Compute loss on batch
            # Input:
                - y_true (3d tf tensor): BxWxH - 0 or 1
                - y_pred (4d tf tensor): BxWxHxC - already apply softmax
            # Output:
        ...
        ## downgrade background class by scale times (0 <= scale <= 1)
...
class FocalLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        """
            ...
            # Input:
                - y_true (3d tf tensor): BxWxH - 0 or 1
                - y_pred (4d tf tensor): BxWxHx2 - already apply softmax
            # Output:
                - focal loss: a scalar
        """
        ## one-hot
        ...
        ## cross entropy loss
        ce_loss = tf.cast((- y_true * tf.math.log(y_pred)), dtype=tf.float32) # BxWxHx2
...
class IOULoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        """
            ...
            # Input:
                - y_true (3D tf tensor): BxWxH - integer class labels
        ...
        y_pred  = tf.reshape(y_pred, [-1, self.num_classes]) # (B*W*H,2)
        ...
        iou_loss = -tf.math.log(iou) # 1 - mean_iou
...
class TverskyLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        tversky_loss = -tf.math.log(tversky_coeff) # 1 - tversky_coeff
